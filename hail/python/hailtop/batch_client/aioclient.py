...
class UpdateBatchBuilder(BatchBuilder):
    ...
    async def batch(self) -> Batch:
        return await self._client.get_batch(self.batch_id)
    ...
    async def _update_commit(self, byte_job_specs: List[bytes], n_jobs: int, pbar):
        assert n_jobs == len(self._job_specs)
        b = bytearray()
        b.extend(b'{"bunch":')
        b.append(ord('['))
        ...
        self.start_job_id = update_json['start_job_id']
        pbar.update(n_jobs)
    ...
    async def submit(self,
        ...
        with tqdm(total=len(self._job_specs),
                  ...
                  desc='jobs submitted to queue') as pbar:
            if len(byte_job_specs_bunches) == 1:
                await self._update_commit(byte_job_specs_bunches[0], bunch_sizes[0], pbar)
            else:
        ...
        for j in self._jobs:
            j._job = j._job._submit(self.batch)
        ...
        self._submitted = True
        return (self.update_id, self.start_job_id)
