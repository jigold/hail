...
BEGIN
  IF OLD.migrated = 0 AND NEW.migrated = 1 THEN
    INSERT INTO job_groups (batch_id, job_group_id, cancel_after_n_failures, path, `state`, n_jobs, time_created, time_completed, callback)
    VALUES (NEW.id, 1, NEW.cancel_after_n_failures, '/', NEW.state, NEW.n_jobs, NEW.time_created, NEW.time_completed, NEW.callback);
