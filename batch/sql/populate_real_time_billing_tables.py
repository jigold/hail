...
from hailtop.utils import check_shell, secret_alnum_string
...
async def execute_sql_command_in_cli(sql: str, config_file: str):
    with tempfile.NamedTemporaryFile('w') as script:
        script.write(sql + '\n')
        await check_shell(
            f'''
mysql --defaults-extra-file={config_file} <{script}
'''
        )
...
async def main():
    db = Database()
    mysql_config_file = '/home/jigold/mysql-config-batch-dump-1'
    json_config_file = '/home/jigold/batch-dump-1-config.json'
    await db.async_init(config_file=json_config_file, cursorclass=aiomysql.cursors.SSDictCursor)
    ...
    try:
        ...
        async def populate(tx):
            ...
            await tx.just_execute('LOCK TABLE attempts WRITE;')

            sql = f'''
{create_tables_sql(token)}

DELIMITER $$

{create_attempt_resources_tmp_after_insert_trigger(token)}
{create_attempts_after_update_trigger(token)}

DELIMITER ;
'''
            await execute_sql_command_in_cli(sql, mysql_config_file)

            # FIXME: Make sure left join keeps all records on the left even if no resources
            records = tx.select_and_fetchall(
                '''
...
''')
        ...
        print(f'exception occurred after {time.time() - start_time}s; reverting triggers and deleting tables')

        @transaction(db)
        async def revert(tx):
            await tx.just_execute('LOCK TABLES attempts WRITE;')
#         sql = f'''
# {revert_triggers_sql(token)}
# {revert_create_tables_sql(token)}
# '''
#         await execute_sql_command_in_cli(sql, mysql_config_file)
            await tx.just_execute('UNLOCK TABLES;')
        await revert()
