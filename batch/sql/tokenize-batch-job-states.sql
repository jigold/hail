...
ALTER TABLE batches_n_jobs_in_complete_states ADD COLUMN token INT DEFAULT 0;
ALTER TABLE batches_n_jobs_in_complete_states ADD COLUMN time_completed BIGINT DEFAULT NULL;
ALTER TABLE batches_n_jobs_in_complete_states DROP PRIMARY KEY, ADD PRIMARY KEY(`id`, token);
...
BEGIN
  ...
  ELSEIF cur_job_state = 'Ready' OR cur_job_state = 'Creating' OR cur_job_state = 'Running' THEN
    ...
    INSERT INTO batches_n_jobs_in_complete_states (`id`, token, time_completed, n_completed, n_cancelled, n_failed, n_succeeded)
    VALUES (in_batch_id, rand_token, new_timestamp,
      1,
      (new_state = 'Cancelled'),
