...
CREATE TABLE IF NOT EXISTS `batches_inst_coll_staging` (
  ...
  FOREIGN KEY (`update_id`) REFERENCES batch_updates(`update_id`) ON DELETE CASCADE,
  FOREIGN KEY (`inst_coll`) REFERENCES inst_colls(name) ON DELETE CASCADE
) ENGINE = InnoDB;
...
BEGIN
  ...
  DECLARE cur_update_cancelled BOOLEAN;
  ...
  DECLARE cur_user VARCHAR(100);
  DECLARE cur_total_n_jobs_in_batch INT;
  DECLARE cur_other_updates_in_progress INT;

  START TRANSACTION;

  SELECT committed, n_jobs, start_job_id, end_job_id, cancelled INTO
    cur_update_committed, expected_n_update_jobs, cur_update_start_job_id, cur_update_end_job_id, cur_update_cancelled
  FROM batch_updates
  ...
  IF cur_update_committed THEN
    COMMIT;
    SELECT 0 as rc;
  ELSEIF NOT cur_update_cancelled THEN
    ...
    SELECT user, n_jobs INTO cur_user, cur_total_n_jobs_in_batch FROM batches WHERE id = in_batch_id;

    IF staging_n_jobs = expected_n_jobs THEN
      IF NOT cur_other_updates_in_progress THEN
        IF cur_total_n_jobs_in_batch = 0 THEN
          UPDATE batches SET `state` = 'complete',
            time_completed = in_timestamp,
  ...
  ELSE
    ROLLBACK;
    SELECT 3 as rc, 'update previously cancelled' as message;
  END IF;
END $$
