-- Testing only!
-- DROP INDEX `batches_format_version` ON `batches`;
CREATE INDEX `batches_format_version` ON `batches` (`format_version`);

-- Testing only!
-- DROP INDEX `aggregated_billing_project_resources_token` ON `aggregated_billing_project_resources`;
CREATE INDEX aggregated_billing_project_resources_token ON `aggregated_billing_project_resources` (`token`);

-- Testing only!
-- DROP INDEX `aggregated_batch_resources_token` ON `aggregated_batch_resources`;
