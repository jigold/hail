...
def test_simple(client):
    ...
    status = batch.wait()
    assert status['job_states']['Complete'] == 2
...
def test_dag(client):
    ...
    status = batch.wait()
    assert status['job_states']['Complete'] == 4
...
def test_cancel_tail(client):
    ...
    tail.cancel()
    status = batch.wait()
    assert status['job_states']['Complete'] == 3
...
def test_one_of_two_parent_ids_cancelled(client):
    ...
    left.cancel()
    status = batch.wait()
    assert status['job_states']['Complete'] == 1
    assert status['job_states']['Cancelled'] == 2
