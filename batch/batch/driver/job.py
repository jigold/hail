...
async def notify_batch_job_complete(db: Database, client_session: httpx.ClientSession, batch_id):
    records = db.select_and_fetchall(
        '''
SELECT job_groups.*, batches.`user`, batches.billing_project, batches.token, batches.msec_mcpu, COALESCE(SUM(`usage` * rate), 0) AS cost,
...
''',
    ...
    async for record in records:
        ...
        log.info(f'making callback for batch {batch_id} job group {record["path"]}: {callback}')

        try:
            if record['user'] == 'ci':
                # only jobs from CI may use batch's TLS identity
                await client_session.post(callback, json=data)
            else:
                async with aiohttp.ClientSession(
                    raise_for_status=True, timeout=aiohttp.ClientTimeout(total=5)
                ) as session:
                    await session.post(callback, json=data)
            log.info(f'callback for batch {batch_id} job group {record["path"]} successful')
        except asyncio.CancelledError:
            raise
        except Exception:
            log.info(f'callback for batch {batch_id} job group {record["path"]} failed, will not retry.')
