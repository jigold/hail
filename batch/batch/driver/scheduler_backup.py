...
class Scheduler:
    def __init__(self, app, instance_group):
        self.app = app
        self.inst_group = instance_group
    ...
    async def compute_fair_share(self):
        free_cores_mcpu = sum([
            ...
            for worker in self.inst_group.healthy_instances_by_free_cores
        ])
...
LIMIT %s;
''',
                            (batch['id'], self.inst_group.id, remaining.value),
