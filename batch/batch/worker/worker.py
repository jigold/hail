        ...
        try:
            ...
            async def localize_rootfs():
                async with image_lock.reader_lock:
                    ...
                    worker.images[self.image_id] += 1

                    self.rootfs_path = f'/host/rootfs/{self.image_id}'
                    async with worker.images[self.image_id].lock:
...
class ImageData:
    ...
    def __add__(self, other):
        self.cache.image_data.remove()
        self.ref_count += other
        self.last_accessed = time_msecs()
        self.cache.image_data.add()

    def __sub__(self, other):
        self.cache.image_data.remove()
        self.ref_count -= other
        assert self.ref_count >= 0, f'{self}'
        self.last_accessed = time_msecs()
        self.cache.image_data.add()

    def __str__(self):
        return f'ImageData(id={self.id} ref_count={self.ref_count} time_created={time_msecs_str(self.time_created)} last_accessed={time_msecs_str(self.last_accessed)})'


class ImageCache:
    def __init__(self):
        self.name_image_data: Dict[str, ImageData] = {}
        self.image_data = sortedcontainers.SortedSet(key=lambda image_data: (image_data.ref_count, image_data.time_created))

    def first_image(self):
        for image_data in self.image_data:
            if image_data.id == BATCH_WORKER_IMAGE_ID:
                continue
            return image_data
        return None

    def __getitem__(self, id):
        if id not in self.name_image_data:
            image_data = ImageData(self, id)
            self.name_image_data[id] = image_data
            self.image_data.add(image_data)
        return self.name_image_data[id]

    def __delitem__(self, id):
        image = self.name_image_data[id]
        self.image_data.remove(image)
        del self.name_image_data[id]

    def __len__(self):
        return len(self.name_image_data)

    def __str__(self):
        return f'{self.image_data}'
...
class Worker:
    def __init__(self):
        ...
        self.images[BATCH_WORKER_IMAGE_ID] += 1
    ...
    async def cleanup_old_images(self):
        try:
            async with image_lock.writer_lock:
                log.info(f"Obtained writer lock. The image cache is: {self.images}")
                first_image = self.images.first_image()
                if first_image:
                    now = time_msecs()
                    if first_image.ref_count == 0 and (now - first_image.last_accessed) > 10 * 60 * 1000:
