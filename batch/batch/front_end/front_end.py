...
async def _query_batch_jobs(request, batch_id, job_group_id, q: str):
    state_query_values = {
        'pending': ['Pending'],
        'ready': ['Ready'],
        'creating': ['Creating'],
...
''',
    ...
    jobs, last_job_id = await _query_batch_jobs(request, batch_id, 1, q)
    resp = {'jobs': jobs}
    if last_job_id is not None:
...
''',
    ...
    for spec in job_specs:
        ...
        async def create_job_group(tx):
            try:
                return await _create_job_group(job_group_cache, tx, batch_id, job_group_path_str, token=None)
            except InvalidJobGroupPathError as e:
                raise e.http_response()

        job_group = await create_job_group()  # pylint: disable=no-value-for-parameter
...
async def update_batch_fast(request, userdata):
    ...
    @transaction(db)
    async def _create_job_groups(tx):
        for jg in job_groups['groups']:
            await _create_job_group(
                job_group_cache,
                tx,
    ...
    try:
        await _create_job_groups()  # pylint: disable=no-value-for-parameter
    except BatchUserError as e:
        return e.http_response()
