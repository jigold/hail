...
class Interval(object):
    ...
    def __init__(self, start, end, include_start=True, include_end=False):
        from hail.expr.expression import impute_type, unify_types_limited
        start_type = impute_type(start)
        end_type = impute_type(end)
        point_type = unify_types_limited(start_type, end_type)

        if point_type is None:
            raise TypeError("'start' and 'end' have incompatible types: %s, %s." % (start_type, end_type))
    ...
    def __repr__(self):
        return 'Interval(start=%s, end=%s, include_start=%s, include_end=%s)' % \
               (repr(self.start), repr(self.end), repr(self.include_start), repr(self._include_end))
    ...
    def contains(self, value):
        ...
        from hail.expr.expression import impute_type
        value_type = impute_type(value)
        if value_type != self.point_type:
            raise TypeError("'value' is incompatible with the interval point type: %s, %s" % (value_type, self.point_type))
    ...
    def overlaps(self, interval):
        ...
        if self.point_type != interval.point_type:
            raise TypeError("'interval' must have the point type %s, but found %s." % (self.point_type, interval.point_type))
        return self._jrep.probablyOverlaps(self.point_type._jtype.ordering(), interval._jrep)
