...
class InstancePool:
    def __init__(self, driver, worker_type, worker_cores, worker_disk_size_gb, pool_size, max_instances):
        self.driver = driver
...
docker run -v /var/run/docker.sock:/var/run/docker.sock -v /usr/bin/docker:/usr/bin/docker -v /batch:/batch -p 5000:5000 -d --entrypoint "/bin/bash" $BATCH_IMAGE -c "sh /run-worker.sh"
'''
                }, {
                    'key': 'inst_token',
                    'value': inst_token
